...
from medmnistutils.medmnistinfo import INFO, HOMEPAGE
...
class MedMNIST(Dataset):
    ...
    def __init__(self,
        ...
        self.size = npz_file['train_images'].shape[0]
        
        self.in_channels = self.info['n_channels']
        ...
        if self.split == 'train':
            ...
            self.labels_unsqueezed = npz_file['train_labels']     
            self.labels = self.labels_unsqueezed.squeeze()  
          
            self.cleanlabels = self.labels.copy()  
            
            

            #self.size = self.labels.shape[0]  
            for i in range(self.size-1):
                tempdice = random.random()
                if tempdice < self.noise_rate:
                    tempint = random.randint(0,self.num_classes-1)      
                    while self.labels[i] == tempint:
                        tempint = random.randint(0,self.num_classes-1)    
                    self.labels[i] = tempint
            self.noisylabels = self.labels.copy()
            

            self.noise_or_not = (self.cleanlabels==self.noisylabels).tolist() 
            self.train_noisy_labels = self.noisylabels.tolist()
